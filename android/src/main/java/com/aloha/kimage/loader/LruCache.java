...
public class LruCache {
    ...
    public LruCache(int maxSize) {
        ...
        keysToRemove = new ArrayList<>(maxSize);
    ...
    private void trimToSize(int maxSize, String justAdded) {
        synchronized (this) {
            Iterator<HashMap.Entry<String, byte[]>> iterator = map.entrySet().iterator();
            while (iterator.hasNext()) {
                if (size <= maxSize || map.isEmpty()) {
                    break;
                }
                HashMap.Entry<String, byte[]> entry = iterator.next();
     ...
     * @return the previous value mapped by {@code key}.
     */
    public final byte[] remove(String key) {
        if (key == null) {
            throw new NullPointerException("key == null");
        }
    ...
    public final byte[] removeByPrefix(String prefix) {
        if (prefix == null) {
    ...
    private int safeSizeOf(String key, byte[] value) {
